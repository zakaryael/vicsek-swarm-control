...
class Potential:
    ...
    def compute(self, positions):
        ...
        '''
        return - 2 * self.alpha * (positions.T-self.loc).T * self.A * np.exp(-self.alpha * np.sum((positions.T - self.loc)**2, axis=1))
    
    def compute2(self, x, y):
        '''computes the values of the potential at position (x,y)
        '''
        return self.A * np.exp(-self.alpha * ((x - self.loc[0])**2 + (y - self.loc[1])**2))

class Swarm:
    def __init__(self, positions, orientations, vel_magnitude, sensing_radius, noise, L, potential=None, save_json=False, save_csv=True):
        ...
        self.velocities = vel_magnitude * np.vstack((np.cos(orientations), np.sin(orientations)))
    ...
    def compute_orientations(self):
        for key, value in self.neighbors.items():
            self.orientations[key] = self.orientations[value].mean() + self.noise * np.random.uniform(-np.pi, np.pi, size=1)
    ...
    def update_orientations(self):
        ...
        dist = tree.sparse_distance_matrix(tree, max_distance=self.sensing_radius,output_type='coo_matrix')
        data = np.exp(self.orientations[dist.col]*1j)
        neigh = sparse.coo_matrix((data,(dist.row,dist.col)), shape=dist.get_shape())
        S = np.squeeze(np.asarray(neigh.tocsr().sum(axis=1)))
        self.orientations = np.angle(S) + self.noise * np.random.uniform(-np.pi, np.pi, size=self.size)
    ...
    def update_positions(self):
        ...
        if self.potential is None:
            ...
            velocity = self.vel_magnitude * np.vstack((vx, vy)) + self.potential.compute(self.positions)
            self.positions += velocity
        ...
        vx, vy = vx / np.linalg.norm(velocity, axis=0), vy / np.linalg.norm(velocity, axis=0)
        self.vx = vx
        self.vy = vy
    ...
    def save_to_csv(self):
        ...
        headers = [f"x_{i}" for i in range(self.size)] + [f"y_{i}" for i in range(self.size)] + \
                    [f"vx_{i}" for i in range(self.size)] + [f"vy_{i}" for i in range(self.size)] + \
                    [f"theta_{i}" for i in range(self.size)] + ["order_param"]

        #create a list of the values to save:
        data = list(self.positions.flatten()) + list(self.velocities.flatten()) + \
            list(self.orientations) + [self.compute_order_param()]
