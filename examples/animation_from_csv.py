...
plt.colorbar()
qv = ax.quiver(df.iloc[0, 0:N], df.iloc[0, N:2*N], np.cos(df.iloc[0, 4*N:5*N]), np.sin(df.iloc[0, 4*N:5*N]), color = 'grey', clim=[-np.pi, np.pi], headaxislength=10, headlength=9)
ax.set_xlim(0, L)
ax.set_ylim(0, L)
plt.tight_layout()
